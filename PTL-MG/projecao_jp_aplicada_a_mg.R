...
PTL<-ts(na.exclude(na.exclude( DADOS$`PTL MG`)[-c(1:12)]),start = c(2007,04),frequency = 12)
PTL_lag12 <-ts(na.exclude( DADOS$`PTL MG`),start = c(2007,04),frequency = 12)
ICP_lag3 <-ts(na.exclude( DADOS$ICP[-c(1:9)]),start = c(2007,04),frequency = 12)
farm_gat_lag4 <-ts(na.exclude( DADOS$`Preço ao produtor deflacionado icp`[-c(1:8)]),start = c(2007,04),frequency = 12)

datatrain<-ts.intersect(PTL,PTL_lag12,ICP_lag3,farm_gat_lag4,dframe = TRUE)
...
farm_gat_lag4prev <-farm_gat_lag4[(length(PTL)+1):(length(farm_gat_lag4))]
PTL_lag12prev <-PTL_lag12[(length(PTL) + 1):(length(farm_gat_lag4))]
trendprev<-(length(PTL)+1):(length(farm_gat_lag4))

matPrev<-matrix( c(ICP_lag3prev,farm_gat_lag4prev,PTL_lag12prev,trendprev),ncol = 4)
colnames(matPrev)<-c("ICP_lag3","farm_gat_lag4","PTL_lag12","trendtrains")
matPrev<-as.data.frame(matPrev)
pred<-predict.glm(tsglm,newdata =matPrev)
write.csv2(pred,"pred_mg.csv")
